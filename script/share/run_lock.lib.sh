...
uri_db_lock() {
    ...
    exec 9> "$lock_file" || { echo "Error: cannot open lock file '$lock_file', exit" >&2; exit 1; }
    flock -n 9 || { echo "Error: another instance working on URI_DB, exit" >&2; exit 1; }
}

tr_db_lock() {
    local lock_file="/run/lock/tr_db.lock"
    exec 10> "$lock_file" || { echo "Error: cannot open lock file '$lock_file', exit" >&2; exit 1; }
    flock -n 10 || { echo "Error: another instance working on TR_DB, exit" >&2; exit 1; }
}
