...
# 0 * * * * telegram_gateway /usr/local/bin/service/xray_stat.sh
...
TMP_TR_DB_M_NEW="$(mktemp)"
TMP_TR_DB_Y_NEW="$(mktemp)"
...
if ! jq -e empty "$TMP_TR_DB_COMMON" &> /dev/null; then
...
if [[ -s "$TR_DB_M" ]] && jq -e empty "$TR_DB_M" &> /dev/null; then
    run_and_check "copy old stat to tmp file" install_new_tr_db "$TR_DB_M" "$TMP_TR_DB_M_OLD"
else
    run_and_check "old stat not valid or empty, start stat from 0 in TR_DB_M" printf '{"stat":[]}\n' > "$TR_DB_M"
    run_and_check "old stat not valid or empty, set stat to 0 in TMP_TR_DB_M_OLD file" printf '{"stat":[]}\n' > "$TMP_TR_DB_M_OLD"
fi

# if old Y empty or not valid, start stat from 0
if [[ -s "$TR_DB_Y" ]] && jq -e . &> /dev/null < "$TR_DB_Y"; then
    run_and_check "copy old stat to tmp file" install_new_tr_db "$TR_DB_Y" "$TMP_TR_DB_Y_OLD"
else
    run_and_check "old stat not valid or empty, start stat from 0 in TR_DB_Y" printf '{"stat":[]}\n' > "$TR_DB_Y"
    run_and_check "old stat not valid or empty, set stat to 0 in TMP_TR_DB_Y_OLD file" printf '{"stat":[]}\n' > "$TMP_TR_DB_Y_OLD"
fi
