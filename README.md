...
- Подготовленный в конфигурационном файле - новое имя сервера, новый пароль, ник для администратора сервера, телеграмм бот token, id кому бот будет слать сообщения, dest сайт под который маскируемся, одно имя для генерации конфигурации впн, количество дней которые действует это имя (0 - бесконечно).
...
мониторинг ноды
добавить место на диске в мониторинг статус

хук присылающий результат обновления сертификата
с переопределением таймера обновления

трафик анблок стилистика
трафик блок уведомление в телегу доделать на основе тайм блок

выбор темы установки

проверить юзер генерацию и добавление в судоерс
